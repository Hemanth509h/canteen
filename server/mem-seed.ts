...
export async function seedMemoryDatabase() {
  const existingCompany = await storage.getCompanyInfo();
  const existingFoodItems = await storage.getFoodItems();

  if (existingCompany && existingFoodItems.length > 0) {
  ...
  if (!existingCompany) {
    ...
    await storage.createCompanyInfo(defaultCompany);
  ...
  if (existingFoodItems.length === 0) {
    ...
    for (const item of comprehensiveFoodItems) {
