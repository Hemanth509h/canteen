...
export default function StaffManager() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({ title: "Staff Added", description: `${data.name} added successfully` });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/staff"] });
      toast({ title: "Removed", description: "Staff removed successfully" });
  ...
  const filteredStaffList = staffList?.filter((staff) => {
    return staff.name.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold font-poppins">Staff Management</h2>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => { setEditingStaff(null); form.reset(); setIsDialogOpen(true); }}>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={staff.id}>
                  ...
                  <TableCell>
                    <Badge variant={roleColors[staff.role] || "secondary"}>{roleMap[staff.role] || staff.role}</Badge>
                  </TableCell>
                  <TableCell>{staff.phone}</TableCell>
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => handleEdit(staff)}><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="text-destructive" onClick={() => { setStaffToDelete(staff); setDeleteConfirmOpen(true); }}><Trash2 className="h-4 w-4" /></Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Remove Staff"
        description={`Are you sure you want to remove ${staffToDelete?.name}?`}
      />
    </div>
  );
}
