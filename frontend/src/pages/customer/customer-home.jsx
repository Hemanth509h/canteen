...
import {
  ShoppingCart, Search, Star, Phone, Mail, MapPin, Instagram,
  Facebook, Twitter, ChevronRight, Plus, CheckCircle, Lock,
  Utensils, Clock, Award, Users, ChefHat
} from "lucide-react";
...
function Navbar({ companyName, logoSrc }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-zinc-950/95 border-b border-zinc-100 dark:border-zinc-800 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2.5">
          {logoSrc && <img src={logoSrc} alt="logo" className="w-8 h-8" />}
          <span className="font-playfair font-bold text-lg text-zinc-900 dark:text-white hidden sm:block">
            {companyName || "Elite Catering"}
          </span>
        </div>

        <div className="hidden md:flex items-center gap-8 text-sm font-jakarta font-medium text-zinc-600 dark:text-zinc-400">
          <button onClick={() => document.getElementById("menu")?.scrollIntoView({ behavior: "smooth" })}
            className="hover:text-amber-600 transition-colors">Menu</button>
          <button onClick={() => document.getElementById("about")?.scrollIntoView({ behavior: "smooth" })}
            className="hover:text-amber-600 transition-colors">About</button>
          <button onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
            className="hover:text-amber-600 transition-colors">Contact</button>
        </div>

        <div className="flex items-center gap-3">
          <ThemeToggle />
          <Link href="/customer/dashboard">
            <button className="text-sm font-jakarta font-semibold text-zinc-600 dark:text-zinc-300 hover:text-amber-600 transition-colors hidden sm:block">
              My Bookings
            </button>
          </Link>
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-zinc-950">
      ...
      <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-400 text-xs font-jakarta font-semibold tracking-widest uppercase mb-8">
          <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
          Premium Catering Service
        </div>

        <h1 className="text-5xl sm:text-7xl lg:text-8xl font-playfair font-bold text-white leading-[1.05] mb-6">
          {companyName || "Elite Catering"}
        </h1>

        <p className="text-lg sm:text-xl text-zinc-400 font-jakarta font-light max-w-2xl mx-auto mb-12 leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-jakarta font-bold text-base transition-all duration-200 hover:scale-105 shadow-lg shadow-amber-500/25"
          >
            Explore Menu <ChevronRight size={18} />
          </button>
          <button
            onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
            className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl border border-zinc-700 text-zinc-300 hover:border-zinc-500 hover:text-white font-jakarta font-semibold text-base transition-all duration-200"
          >
            Book an Event
          </button>
        </div>

        <div className="flex items-center justify-center gap-8 mt-16 pt-12 border-t border-zinc-800">
          {[
            ...
            <div key={stat.label} className="text-center">
              <div className="text-2xl sm:text-3xl font-playfair font-bold text-amber-400">{stat.value}</div>
              <div className="text-xs text-zinc-500 font-jakarta mt-0.5">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
...
function Features() {
  ...
  return (
    <section id="about" className="bg-white dark:bg-zinc-900 py-16 px-6 border-b border-zinc-100 dark:border-zinc-800">
      <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-6">
        {items.map((f) => (
          <div key={f.title} className="flex flex-col items-center text-center gap-3 group">
            ...
            <div>
              <p className="font-playfair font-bold text-zinc-900 dark:text-white text-base">{f.title}</p>
...
function MenuSection({ foodItems, isLoading, onSelectItem, addToCart, cartItems }) {
  ...
  return (
    <section id="menu" className="bg-zinc-50 dark:bg-zinc-950 py-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl sm:text-5xl font-playfair font-bold text-zinc-900 dark:text-white mb-4">Seasonal Selections</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-11 pl-11 pr-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-sm font-jakarta text-zinc-900 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-amber-500/30"
            />
          </div>
        </div>
    ...
    <section className="bg-white dark:bg-zinc-900 py-20 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <h2 className="text-4xl sm:text-5xl font-playfair font-bold text-zinc-900 dark:text-white">What Clients Say</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {list.map((r, i) => (
            <div key={i} className="p-6 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-700/50 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 pt-2 border-t border-zinc-200 dark:border-zinc-700">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 font-jakarta">{r.eventType}</p>
                </div>
              </div>
            </div>
          ))}
...
function Footer({ companyInfo, logoSrc }) {
  return (
    <footer id="contact" className="bg-zinc-950 text-zinc-400 py-16 px-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div>
            ...
            <div className="flex gap-3 mt-5">
              {[Instagram, Facebook, Twitter].map((Icon, i) => (
                <button key={i} className="w-9 h-9 rounded-full border border-zinc-800 hover:border-amber-500 hover:text-amber-400 transition-colors flex items-center justify-center">
                  <Icon size={15} />
                </button>
              ))}
            </div>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-jakarta">
          ...
          <a href={`tel:${companyInfo?.phoneNumber || ""}`} className="flex items-center gap-2 text-amber-400 font-semibold hover:text-amber-300 transition-colors">
...
export default function CustomerHome() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-900 dark:text-white">
      ...
      {/* Floating phone */}
      {companyInfo?.phoneNumber && (
        <a href={`tel:${companyInfo.phoneNumber.replace(/\D/g, "")}`}
          className="fixed bottom-6 left-6 z-50 w-14 h-14 rounded-full bg-amber-500 text-zinc-950 flex items-center justify-center shadow-lg shadow-amber-500/30 hover:scale-110 transition-transform">
          <Phone size={22} />
        </a>
      )}

      <Dialog open={!!selectedItem} onOpenChange={(o) => !o && setSelectedItem(null)}>
        <DialogContent className="max-w-4xl p-0 overflow-hidden border-zinc-200 dark:border-zinc-800 rounded-3xl bg-white dark:bg-zinc-900">
          <FoodItemQuickView
            item={selectedItem}
